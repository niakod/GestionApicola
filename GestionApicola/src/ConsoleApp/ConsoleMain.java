...
public class ConsoleMain {
	...
	private static void registrarApicultor(Scanner s) throws IOException{
		...
		try{
			...
			TipoRetorno ret = Sistema.getInstancia().registrarApicultor(cedula, nombre, direccion, email, celular);
			//Resultado la funcionalidad:
			System.out.println(ret.getTipoError().toString());
			if (ret.getTipoError() == TipoError.ERROR_1){
				System.out.println("Ya existe un apicultor con la cédula "+cedula+" registrado en el sistema.");
			}
			System.in.read();
		}
		catch(InputMismatchException ime){
